import sys
from typing import Optional  # Noneが入る可能性がある場合
# なかなか理解が難しい項目 何回も繰り返し読み込んで書いて理解したほうがいい...
# インタビュークイズでは出にくいかも...


class MiniHeap(object):

    def __init__(self) -> None:
        self.heap = [-1 * sys.maxsize]
        self.current_size = 0

    def parent_index(self, index: int) -> int:
        return index // 2

    def left_child_index(self, index: int) -> int:
        return 2 * index

    def right_child_index(self, index: int) -> int:
        return 2 * index + 1

    def swap(self, index1: int, index2: int) -> None:  # 入れ替え
        self.heap[index1], self.heap[index2] = self.heap[index2], self.heap[index1]

    def heapify_up(self, index: int) -> None:
        while self.parent_index(index) > 0:
            if self.heap[index] < self.heap[self.parent_index(index)]:
                self.swap(index, self.parent_index(index))
            index = self.parent_index(index)

    def push(self, value: int) -> None:
        self.heap.append(value)
        self.current_size += 1
        self.heapify_up(self.current_size)

    def min_child_index(self, index: int) -> int:
        # 左と右を比べる際に、左しかない場合
        if self.right_child_index(index) > self.current_size:
            return self.left_child_index(index)
        else:  # 左を右を比べる際
            if (self.heap[self.left_child_index(index)] <
                    self.heap[self.right_child_index(index)]):
                return self.left_child_index(index)
            else:
                return self.right_child_index(index)

    def heapify_down(self, index: int) -> None:
        while self.left_child_index(index) <= self.current_size:
            min_child_index = self.min_child_index(index)
            if self.heap[index] > self.heap[min_child_index]:
                self.swap(index, min_child_index)
            index = min_child_index

    def pop(self) -> Optional[int]:
        if len(self.heap) == 1:
            return

        root = self.heap[1]  # 一番小さい値
        data = self.heap.pop()  # 一番最後の値

        if len(self.heap) == 1:
            return root

        # [-x,1,6,2,9,13,11] popで取り出した後、再構成するコード
        self.heap[1] = data  # 先頭に持ってくる
        self.current_size -= 1
        self.heapify_down(1)
        return root


if __name__ == '__main__':
    min_heap = MiniHeap()
    min_heap.push(5)
    min_heap.push(6)
    min_heap.push(10)
    min_heap.push(13)
    min_heap.push(1)
    min_heap.push(2)
    print(min_heap.heap)
    print(min_heap.pop())
    print(min_heap.heap)
